...
fn parser_code_101_then_401() {
    ...
    fs::copy("./tests/input_files/parser/101_then_401.json", &file_path).unwrap();
    ...
    let mut expected_result_file = File::open("./tests/expected_files/parser/101_then_401.txt").unwrap();
...
fn parser_code_102_then_402() {
    ...
    //Copy over the needed files
    fs::copy("./tests/input_files/parser/102_then_402.json", &file_path).unwrap();
    ...
    //See if the files are the same
    let mut expected_result_file = File::open("./tests/expected_files/parser/102_then_402.txt").unwrap();
...
fn parser_escaped_quotes() {
    ...
    let file_path = input_dir.join("escaped_quotes.json");
    
    //Copy over the needed files
    fs::copy("./tests/input_files/parser/escaped_quotes.json", &file_path).unwrap();
    ...
    //See if the files are the same
    let mut expected_result_file = File::open("./tests/expected_files/parser/escaped_quotes.txt").unwrap();
    let mut actual_result_file = File::open(&patch_dir.join("escaped_quotes.txt")).unwrap();
